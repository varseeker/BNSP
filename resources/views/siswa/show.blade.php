    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <form action="{{ $siswa->nis }}" method="POST" class="d-inline float-right">
                    @method('delete')
                    @csrf
            ...
            <div class="card-body">
                ...
                <p class="card-text">{{ $siswa->nmsis }}</p>

                {{-- JENIS KELAMIN --}}
                <h5 class="card-title">Jenis Kelamin</h5>
                <p class="card-text">{{ $siswa->jksis }}</p>
                ...
                <p class="card-text">{{ $siswa->nhnajar }}</p>

                {{-- WALI SISWA --}}
                <h5 class="card-title">Wali Siswa</h5>
                <p class="card-text">{{ $siswa->wali }}</p>

                {{-- NO TELEPHONE/HANDPHONE --}}
                <h5 class="card-title">Tahun Ajar</h5>
                <p class="card-text">{{ $siswa->tlpwl }}</p>
            </div>
        </div>
    </div>
    <!-- Optional JavaScript -->
